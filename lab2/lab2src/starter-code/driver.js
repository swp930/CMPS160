...
function move(ev, gl, canvas) {
    ...
    g_points = g_points.splice(0,g_points.length-4)
...
function click(ev, gl, canvas) {
	...
	gl.clear(gl.COLOR_BUFFER_BIT);
      ...
      var last = -1
      var arr = []
      for(i = 0; i+4 < g_points.length; i+=4){
...
function drawCylinder(point1, point2, gl){
  var arr = getCirclePoint(point1)
  var arr2 = getCirclePoint(point2)
...
function getCirclePoint(center){
  var arr = []
  var i = 0
  for(i = 1; i <= 12; i++){
    arr.push(0.05*Math.cos(toRadians(i*30))+center[0])
    arr.push(center[1])
    arr.push(0.05*Math.sin(toRadians(i*30)))
  }
  return arr
}
