...
function start(gl, canvas){
  ...
  document.getElementById('rotateYAxis').onclick = function(ev){ rotateAxis(ev, gl, 1); };
  //document.getElementById('rotateSquare').onclick = function(ev){ rotateSquare(ev, gl); };
  document.getElementById('rotateXAxis').onclick = function(ev){ rotateAxis(ev, gl, 0); };
...
function rotateAxis(ev, gl, axes){
  /*for(i = 0; i+1 < cylinderPoints.length; i+=2){
    var x = cylinderPoints[i]
    var y = cylinderPoints[i+1]
    //var xnew = x*Math.cos(toRadians(5)) -
  }*/
  yDegree += 5
  gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);
  drawAllCylinders(cylinderPoints, gl)
